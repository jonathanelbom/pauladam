(function() {
	 ...
	 *                 thumbs handler
	 *********************************************************/
	$work.on( mouseEvent, '.thumb', function() {
		if ( !scrolling ) {
			showFullImage( findPhotoFromAllPhotos(  $(this).closest('div').data('photoId') ) );
		}
		scrolling = false;
	...
	*/
	...
	function findPhotoFromAllPhotos( photoId ) {
		photo = _.findWhere( allPhotos, {id: photoId.toString()} );
		curPhotos = _photos[ photo.photosetId ];
		return photo;
	}
	...
	function onGetPhotoset(data) {
		...
		// add all photos for this photoset
		addPhotos( _photos[ photoset.id ] );
		// init grid for this photoset
	...
	function onPhotosetsObtained( sets ) {
		//console.log('onPhotosetsObtained, sets:',sets);
